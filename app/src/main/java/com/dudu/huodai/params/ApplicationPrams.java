package com.dudu.huodai.params;

import com.dudu.model.bean.LoginCallBackBean;

public class ApplicationPrams {
    public static LoginCallBackBean.UserBean loginCallBackBean;
    public static boolean isLogin;


    public static final int MainActvity=19;
    public static final int SpecialActivity=20;
    public static final int LabelActivity=21;
    public static final int SubjctActivity=22;

    public static final int GameTree=23;
    public static final int GameCiccle=24;
    public static final int GameGoldEgg=25;
    public static final int GameCai=26;


    public static final String key_title="title";
    public static final String key_id="id";
    public static final String key_tiaozhuan="pid";
    public static final String adverId="advertId";

    public static final String public_restart_advertId ="828300403";//唤醒时候的广告id
    public static final String public_sceenon_advertId ="828300338";//屏幕点亮的广告
    public static final String public_firstpage_advertId="928300923";//首页的广告id
    public static final String public_storybanner_advertId="928300853";//故事详情banner
    public static final String public_qiandao_back="928300698";//签到回归的底部广告
    public static final String public_qiandao_jili="928300970";//签到的激励视频
    public static final String public_qiandao_fanbei="928300698";//签到额翻倍底部广告

    public static final String public_game_cai_kai="828300006";//签到回归的底部广告
    public static final String public_game_circle_kai="828300800";//签到的激励视频
    public static final String public_game_gold_kai="828300479";//签到额翻倍底部广告
    public static final String public_game_tree_kai="828300124";//签到额翻倍底部广告

    public static final String public_game_cai_bottom="928300489";//签到回归的底部广告
    public static final String public_game_circle_bottom="928300524";//签到的激励视频
    public static final String public_game_gold_bottom="928300804";//签到额翻倍底部广告
    public static final String public_game_tree_bottom="928300403";//签到额翻倍底部广告

    public static final String public_game_cai_dialog_bottom="928300758";//签到回归的底部广告
    public static final String public_game_circle_dialog_bottom="928300208";//签到的激励视频
    public static final String public_game_gold_dialog_bottom="928300087";//签到额翻倍底部广告
    public static final String public_game_tree_dialog_bottom="928300329";//签到额翻倍底部广告



    public static final String public_game_cai_suceesee_jili="928300970";//签到回归的底部广告
    public static final String public_game_cai_fail_jili="928300970";//签到回归的底部广告
    public static final String public_game_circle_jili="928300062";//签到的激励视频
    public static final String public_game_gold_jili="928300463";//签到额翻倍底部广告
    public static final String public_game_tree_jili="928300783";//签到额翻倍底部广告


    public static final String public_game_cai_back_dialog_bottom="928300758";//签到回归的底部广告
    public static final String public_game_circle_back_dialog_bottom="928300208";//签到的激励视频
    public static final String public_game_gold_back_dialog_bottom="928300087";//签到额翻倍底部广告
    public static final String public_game_tree_back_dialog_bottom="928300329";//签到额翻倍底部广告
}
